...
# Feature: Vaccination Rates
X = data[['total_vaccinations_per_hundred']]
y = data['Cases']

# Drop rows with NaN values
data = data.dropna(subset=['total_vaccinations_per_hundred', 'Cases'])
X = data[['total_vaccinations_per_hundred']]
y = data['Cases']
...
plt.xlabel('Date')
plt.ylabel('Cases')
plt.title('COVID-19 Cases Forecasting Based on Vaccination Rates (Gradient Boosting)')
plt.legend()
plt.show()